...
送信データはACKされるまでBRAMリングバッファに保持する。32 KiB版では書込み位置、TCPの`snd_nxt`、ACK済み位置を区別し、再送が新しい入力に上書きされないことをRTL試験と実機で確認した。32 KiBは現在の40 MB/sで約0.82 ms分に相当する。RHEA統合時には64 KiB、128 KiBも比較する。既存2段FIFOは最初の統合では残し、安定動作後にコピーや容量を整理する。
