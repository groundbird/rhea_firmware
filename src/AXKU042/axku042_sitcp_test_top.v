...
//   5. ping 192.168.10.16  → ICMP echo reply visible in Wireshark
//   6. nc 192.168.10.16 24  → TCP echo (anything typed is echoed back)
...
//
// Clock plan:
//   PL_CLK0 (200 MHz diff) → IBUFDS → MMCME3_ADV
//     CLKOUT0 (125 MHz, 0°)   → u_bufg_clk125   → clk125   (GMII TX)
//     CLKOUT1 (125 MHz, +90°) → u_bufg_clk125_90 → clk125_90 (PHY_GTXC)
//     CLKOUT2 (200 MHz, 0°)   → u_bufg_clk200    → clk200  (SiTCP CLK)
//   PHY_RXC (125 MHz from PHY) → IBUF → u_bufg_rxc → rxc    (GMII RX)
//
// RGMII:
//   TX : GMII 8-bit @ clk125 → ODDRE1 → 4-bit DDR @ clk125
//        GTX_CLK driven with ODDRE1 @ clk125_90 (+90° ≈ 2 ns delay vs data)
//   RX : 4-bit DDR @ rxc → IDDRE1 (SAME_EDGE_PIPELINED) → GMII 8-bit @ rxc
//------------------------------------------------------------------------------
...
module axku042_sitcp_test_top (
    ...
    // -------------------------------------------------------------------------
    // Parameters – edit here to change IP / ports
    // -------------------------------------------------------------------------
    ...
    // MMCME3_ADV: 200 MHz → 125 MHz (0°), 125 MHz (+90°), 200 MHz (0°)
    // VCO = 200 × 5 = 1000 MHz  (within 600–1200 MHz for xcku040 speed-2)
    MMCME3_ADV #(
        .BANDWIDTH          ("OPTIMIZED"),
        .COMPENSATION       ("ZHOLD"),
        .STARTUP_WAIT       ("FALSE"),
        ...
        .CLKFBOUT_PHASE     (0.0),
        // CLKOUT0: 125 MHz, 0° – TX data clock
        .CLKOUT0_DIVIDE_F   (8.000),
        .CLKOUT0_PHASE      (0.000),
        .CLKOUT0_DUTY_CYCLE (0.5),
        // CLKOUT1: 125 MHz, +90° – GTX_CLK output (data-stable before clock edge)
        .CLKOUT1_DIVIDE     (8),
        .CLKOUT1_PHASE      (90.000),
        .CLKOUT1_DUTY_CYCLE (0.5),
        // CLKOUT2: 200 MHz, 0° – SiTCP system clock
        .CLKOUT2_DIVIDE     (5),
        .CLKOUT2_PHASE      (0.000),
    ...
    // Power-up delay: hold PHY reset low for ~10 ms (200 MHz × 2 000 000 = 10 ms)
    ...
    // =========================================================================
    // RGMII TX – GMII 8-bit (clk125) → 4-bit DDR
    // =========================================================================
    ...
    // PHY_GTXC: forward clk125_90 (+90°) so the PHY sees data-stable before clock
    wire gtxc_pre;
    ...
    // =========================================================================
    // RGMII RX – 4-bit DDR → GMII 8-bit (rxc domain)
    // =========================================================================
    ...
    IOBUF u_iobuf_mdio (
        .I (mdio_out),
        .O (mdio_in),
        .T (~mdio_oe),  // T=1 → tri-state (input), T=0 → drive
        .IO(PHY_MDIO)
    );
    ...
    // TCP echo: RX data → TX data (direct loopback)
